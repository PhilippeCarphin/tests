...
# we press TAB.
# It's purpose is # to examine the current command
# line (as represented by the array COMP_WORDS)
# and to determine what the autocomplete should
# reply through the array
# COMPREPLY.
# This function is organized with subroutines who
# are responsible for setting the 'candidates'
# variable.
# The compgen then filters out the candidates
# that don't begin with the word we are completing.
# In this case, if '--' is one of the words, we
# set empty candidates, otherwise, we look at
# the previous word and delegate to candidate-setting
# functions
################################################################################
__ord_soumet_complete()
{
    ...
    if ! __dash_dash_in_words ; then
        if [[ "$cur" == -* ]] ; then
            candidates="$(__complete_ord_soumet_option)"
        else
            candidates="$(__complete_ord_soumet_posargs)"
        fi
    else
        candidates=""
    fi
...
__complete_ord_soumet_option(){
    echo "-addstep -altcfgdir -args -as -c -clone -cm -coschedule -cpus
