...
/*
 * This test shows the obvious, that integers have a maximum value in C and that
 * when we pass this value, an overflow occurs.  Therefore, when calculating the
 * value of (2^311)%1000, for example, we must take the modulo at the same time
 * as we take powers since (a*b) == (a%n)(b%n) mod n;
 *
 * There is also a python version of this experiment that shows that python can
 * have arbitrarily large integers.
 */
...
int main(void)
{
   printf("First  exp: 3^311 mod 1000 : %lu\n", exponentiate1(3,311,1000));
   printf("Second exp: 3^311 mod 1000 : %lu\n", exponentiate2(3,311,1000));
   return 0;
}
